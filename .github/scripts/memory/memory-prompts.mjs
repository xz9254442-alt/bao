...
export function buildIssueAgentPrompt({
  ...
  return [
    `請以${language}輸出一份高品質 Markdown，內容將會寫入 .memory/agents/issue-${issueThread.number}.md。`,
    ...
    '請直接從 `## Status` 開始輸出，不要再輸出額外標題或前言。',
    '必須包含這些章節：`## Status`、`## Recent Activity`、`## Decisions`、`## Completed Work`、`## Open Tasks`。',
    '- 用繁體中文，語氣像一隻可靠的龍蝦在替主人整理記憶；內容仍要保有工程可接手性。',
    '- 要從 issue title、issue body、Telegram 對話與留言中辨識這隻龍蝦的角色與功能，並以這個角色來記錄事情。',
    '- `## Status` 必須先判斷這個 issue 目前是活躍、阻塞、等待、已完成、或資訊不明，並說明原因。',
    '- `## Recent Activity` 要優先整理最近一輪對話中這隻龍蝦實際幫主人做了什麼、回覆了什麼、目前推進到哪裡。',
    '- `## Decisions` 要列出已經定下來的方向；如果沒有明確決策，要直接說明。',
    '- `## Completed Work` 與 `## Open Tasks` 要分清楚已完成與未完成事項。',
    '- 這份記憶要總結對話過程與工作脈絡，不是單純事件 log；要寫出為什麼這些對話重要。',
    '- 如果某些 Telegram 對話只是確認或閒聊，只保留對後續接手工作有用的資訊。',
    '- 不要輸出 `## Risks` 或 `## Next Steps` 章節。',
    '',
...
export function buildDailyPrompt({
  ...
  const agentBlocks = agentMemories.map((agentMemory) =>
    [
      ...
      '',
      agentMemory.promptSummaryMarkdown || agentMemory.summaryMarkdown,
      '</agent>',
    ].join('\n'),
  );
